...
 
# 5. Информационная безопасность.
Геоинформационная система литературного картирования произведений русской литературы предназначена для загрузки любых текстов любыми пользователями, то есть результаты их обработки видны всем. Все загруженные файлы хранятся на сервере, данные результата алгоритма анализа – в базе данных, доступ к которой только у разработчика. Веб-приложение не несет никакой опасности конечному пользователю, так как не содержит доступ к какой-либо личной информации пользователя. Но это в случае, если пользователь сам не загрузит тексты с личными данными, иначе ему следует сообщить об ошибке (рисунок 16), чтобы разработчик убрал все необходимые файлы и данные из системы.

// тут надо про класс защиты от НСД https://fstec.ru/dokumenty/vse-dokumenty/spetsialnye-normativnye-dokumenty/rukovodyashchij-dokument-ot-4-iyunya-1999-g-n-114
// Возможно понадобятся сертификаты безопасности: https://www.gosuslugi.ru/crt
